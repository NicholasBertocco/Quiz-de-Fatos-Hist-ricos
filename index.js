...
//Embaralhando as perguntas do array
const questoesRandom = questoes.sort(() => Math.random() - 0.5);

//Selecionando 10 perguntas 
const questoesSelect = questoesRandom.slice(0,10);

//Exibindo questoes resposta e vendo se esta certa
function imprimirQuestao (questao){
  console.log(questao.pergunta);
  const respostaUsuario = input.question("Digite a resposta (ano): ");
  return respostaUsuario;
}
 //Validando resposta
function validaRespostadoUsuario (resposta, pergunta){
...
function exibirResultado(pontuacaoFinal, nomeJogador){
  ...
  if (pontuacaoFinal<=3){
    ...
    mensagem = 'EXCELENTE! Voê é um verdadeiro expert.';
  }
...
function iniciarQUIZ (){
  ...
  console.log("\nSeja Bem-vindo novo Jogador(a)!\n");
  const name = input.question("Digite o seu nome: ");

  let pontuacaoFinal = 0;

  questoesSelect.forEach((pergunta) => {
    const resposta = imprimirQuestao(pergunta);
    const resultadoValidacao = validaRespostadoUsuario(resposta, pergunta);
  ...
  exibirResultado(pontuacaoFinal,name);
}
